...
class ChordFromGuitarView(ttk.Frame):
    ...
    def setup_ui(self):
        ttk.Label(self, text="🎸 기타로 루트음을 입력 중...", font=("Helvetica", 18, "bold")).pack(pady=20)
        self.status_label = ttk.Label(self, text="15초간 입력을 감지하고 있습니다...", font=("Helvetica", 12))
        self.status_label.pack(pady=5)

        self.note_frame = ttk.LabelFrame(self, text="결과 확인 및 수정")
        self.note_frame.pack(pady=10)

        self.submit_button = ttk.Button(self, text="확인하고 다음으로", command=self.submit, state="disabled")
    ...
    def capture_notes(self):
        ...
        try:
            ...
            self.after(0, lambda: self.update_note_checkboxes(top))
    ...
    def submit(self):
        selected_notes = [note for note, var in self.note_vars.items() if var.get()]
        if not selected_notes:
            messagebox.showwarning("입력 오류", "최소 하나의 노트를 선택해주세요.")
            return
        self.controller.pass_chords_to_style(selected_notes)