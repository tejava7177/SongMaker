...
class TunerView(ttk.Frame):
    ...
    def setup_ui(self):
        ttk.Label(self, text="🎸 기타 튜너 (Bass)", font=("Helvetica", 18, "bold")).pack(pady=10)

        ttk.Label(self, text="오디오 입력 장치 선택:").pack(pady=5)
        self.device_combobox = ttk.Combobox(self, width=50)
        self.device_combobox.pack(pady=5)
        self.load_devices()

        self.string_buttons_frame = ttk.Frame(self)
        ...
        self.current_note_label = ttk.Label(self, text="튜닝할 줄을 선택하세요", font=("Helvetica", 14))
        self.current_note_label.pack(pady=10)

        self.highlight_note_label = tk.Label(self, text="-", font=("Helvetica", 32, "bold"))
        self.highlight_note_label.pack(pady=10)

        self.detail_label = ttk.Label(self, text="", font=("Helvetica", 12))
        self.detail_label.pack(pady=5)

        self.start_button = ttk.Button(self, text="튜너 시작", command=self.toggle_tuner)
        self.start_button.pack(pady=10)

        self.status_label = ttk.Label(self, text="상태: 대기 중", font=("Helvetica", 12))
    ...
    def run_tuner(self):
        def ui_callback(note, freq, status):
            self.status_label.config(text="상태: 측정 중…")
